...
apktransform {
    copy {
        when (it.buildType) {
            "release" -> file("${it.name}/ink.chyk.pass.${VersionName}.apk")
            "debug" -> file("${it.name}/ink.chyk.pass.${VersionName}-debug.apk")
            else -> null
        }
    }
}
...
android {
    namespace = "ink.chyk.pass"
    compileSdk = 35

    defaultConfig {
        applicationId = "ink.chyk.pass"
        minSdk = 28
